...
const Register = ()=> {
    ...
    const registerSubmit = async() => {
      ...
      if(password && OTP && (phone || email)){
        ...
        const response = await axios.post(`${process.env.REACT_APP_LOCAL_BASE_URL}/api/user/register` , data )
    ...
    return (
      <>
        ...
        <div className='h3 d-flex align-items-center justify-content-center mb-5 '>Register</div>
          ...
          <div className="form-group mt-2">
            <label htmlFor="pwd">OTP Code*</label>
            <input placeholder= {`Check Your ${getLabel()}`} type="text" className="form-control" id="pwd" onChange={(e)=>{setOTP(e.target.value)}}/>
          </div>
          ...
          <button type="submit" className={`btn btn-danger mt-2 p-2 ${OTP? '': 'disabled'}`}
